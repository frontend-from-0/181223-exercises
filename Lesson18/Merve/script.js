...
function concatenateWithSpaces(arr) {
    return arr.join(' ');
}

const output = concatenateWithSpaces(stringE3);
console.log(output);
...
const output1 = hasKey(testObjEx9, key1);
const output2 = hasKey(testObjEx9, key2);
...
const cityKey = "city"; // Renamed variable 'key' to 'cityKey'
const cityValue = "New York"; // Optionally, rename 'value' to 'cityValue' for consistency
const obj = addKeyValuePair(testObjEx10, cityKey, cityValue);
...
const updatedObj = removeKeyValuePair(testObjEx11, keyToRemove);
...
function matchArraysToObj(keys, values) {
    let obj = {};
...
console.log(matchArraysToObj(keys, values));
