...
const numbersExercise2Square = numbersExercise2.map((number) => number ** 2);
...
const names = products.map((user) => user.name);
console.log(names); 
...
const lengthOfWord = wordsExercise4.map((word) => word.length); 
...
const namesAndGrades = studentsExercise5.map((students) => `${students.name} ${students.grade}`);
...
const splitTheSentences = sentencesExercise7.map((words) => words.split());
console.log(splitTheSentences);
...
const emailAdresses = usersExercise8.map((users) => users.email);
...
const titlesAndAuthors = booksExercise9.map((books) => `${books.title} ${books.author}`);
